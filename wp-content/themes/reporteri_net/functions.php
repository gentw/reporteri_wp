...
function zox_register_required_plugins() {
	...
	$config = array(
		...
		'strings'      => array(
			...
			'nag_type'                        => '', // Determines admin napproachotice type - can only be one of the typical WP notice classes, such as 'updated', 'update-nag', 'notice-warning', 'notice-info' or 'error'. Some of which may not work as expected in older WP versions.
		),
		*/
	);
...
function add_new_design_stylesheet() {
    wp_enqueue_style( 'new-stylesheet', get_template_directory_uri() . '/custom_css/newdesign.css' );
}
add_action( 'wp_enqueue_scripts', 'add_new_design_stylesheet' );
...
function insert_adsense_ads($content) {
    ...
    if (is_single() && !is_admin()) {
		...
		ob_start(); 
		include(get_template_directory() . '/parts/' . 'related-news.php');
        $php_file = ob_get_clean();

		if(count($paragraphs) <= 2) {
			$paragraphs[1] .= $php_file;
		} else {
			foreach ($paragraphs as $index => $paragraph) {
				if ($index === $randnr) {
					$paragraphs[$index] .= $php_file;
				}
			}
		}
		$content = implode("\n", $paragraphs);
